...
def query():
    try:
        ...
        if abs(charge) < 0.1:
            model, scaler = garnet.load_model_and_scaler(mix_site) if mix_site else garnet.load_model_and_scaler("c")
...
#
#         if abs(charge) < 0.1:
#             model, scaler = garnet.load_model_and_scaler(
#                 mix_site) if mix_site else garnet.load_model_and_scaler("c")
#             inputs = garnet.get_descriptor_ext(species)
#             form_e = garnet.get_form_e_ext(inputs, model, scaler)
#             tot_e = garnet.get_tote(form_e, species)
#             if mix_site:
...
def perovskite_query():
    try:
        ...
        if abs(charge) < 0.1:
            model, scaler = perovskite.load_model_and_scaler(mix_site) if mix_site else perovskite.load_model_and_scaler("a")
